...
def compute_all_metrics(pred, gt, gt_multi):
    """
    calculate all metrics 计算评价指标

    Args:
        pred: candidate prediction, shape as [50, t_pred, 3 * joints_num] [N,t_pred,3*J]
    ...
    if pred.shape[0] == 1:
        diversity = 0.0
    dist_diverse = torch.pdist(pred.reshape(pred.shape[0], -1))  # 计算p范数距离
    diversity = dist_diverse.mean()

    gt_multi = torch.from_numpy(gt_multi).to('cuda')
    gt_multi_gt = torch.cat([gt_multi, gt], dim=0)

    gt_multi_gt = gt_multi_gt[None, ...]  # 扩充gt_multi_gt第0维
    pred = pred[:, None, ...]  # 扩充pred第1维

    diff_multi = pred - gt_multi_gt
    dist = torch.linalg.norm(diff_multi, dim=3)  # 计算l2范数

    ...
    def run_train_step(self):
        ...
        for traj_np in self.generator_train:
            with torch.no_grad():
                ...
                # discard the root joint and combine xyz coordinate 抛弃第0个节点
                traj_np = traj_np[..., 1:, :].reshape([traj_np.shape[0], self.cfg.t_his + self.cfg.t_pred, -1])
                traj = tensor(traj_np, device=self.cfg.device, dtype=self.cfg.dtype)  # 转tensor
                # last frame padding idx_pad:[0, 1, 2,....,24, 24, 24,.....]
                # 返回填充序列 使用填充，至标准长度125，填充后序列作为条件输入噪声预测模型
                traj_pad = padding_traj(traj, self.cfg.padding, self.cfg.idx_pad, self.cfg.zero_index)
                traj_dct = torch.matmul(self.cfg.dct_m_all[:self.cfg.n_pre], traj)  # 对原始序列做DCT变换,只变换前n_pre行
                # dct_m:[n_pre,t_his + t_pre] 矩阵乘法
                traj_dct_mod = torch.matmul(self.cfg.dct_m_all[:self.cfg.n_pre], traj_pad)  # 对pad后序列做DCT变换
                # 频域变换后的轨迹只剩下前n_pre行 (N, n_pre, 3 * (joints - 1))
                if np.random.random() > self.cfg.mod_train:  # mod_train(hyper para): 0.8
                    traj_dct_mod = None

            # train
            t = self.diffusion.sample_timesteps(traj.shape[0]).to(self.cfg.device)  # 获取一个扩散时间步
            x_t, noise = self.diffusion.noise_motion(traj_dct, t)  # 前向加噪 (N, n_pre, 3 * (joints - 1))
            predicted_noise = self.model(x_t, t, mod=traj_dct_mod)  # 模型预测噪声 (N, n_pre, 3 * (joints - 1))

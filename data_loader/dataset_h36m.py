...
class DatasetH36M(Dataset):
    ...
    def prepare_data(self):
        ...
        self.subjects_split = {'train': [1, 5, 6, 7, 8],
                               'test': [9, 11]}
        self.subjects = ['S%d' % x for x in self.subjects_split[self.mode]]  # 根据索引返回一个subjects列表
        ...
        self.skeleton.remove_joints(self.removed_joints)  # 丢弃部分关节
        self.skeleton._parents[11] = 8
        self.skeleton._parents[14] = 8
        print(self.kept_joints)
        self.process_data()
    ...
    def process_data(self):
        ...
        # 只保留那些主体名称在self.subjects列表中的元组。然后，使用dict函数，将过滤后的元组列表转换回一个字典，赋值给data_f变量
        if self.actions != 'all':
